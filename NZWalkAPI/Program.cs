...
//-------------Added Logger service to the container by Raghvendra for Logging-------------
//var logger = new LoggerConfiguration().WriteTo.Console().MinimumLevel.Information().CreateLogger();  // It will log at Console as Information.
var logger = new LoggerConfiguration().WriteTo.File("Logs/LogFile.txt", rollingInterval:RollingInterval.Day).MinimumLevel.Information().CreateLogger();  // It will log into file as Information.
builder.Logging.ClearProviders();   //Clearing the any other logger.
builder.Logging.AddSerilog(logger);       //Adding the Serilog for logging.
...
//------------Added some code by Raghav to use Authorization Header to the Swagger to byPass Postman-------------
